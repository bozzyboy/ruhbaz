...
export function buildReadingPrompt(params: {
  ...
  const parts = [enOutputLanguageSystemDirective(), sanitizeRestrictedReadingTerms(identity.systemBody), getReadingSafetyCore(), getPersonaSelfNameDirective(id), runtimeRules, buildAddressPolicy(id, params.memorySnippet), buildSafetyPolicy()];
  if (focusQuestion) {
    parts.push(
      [
