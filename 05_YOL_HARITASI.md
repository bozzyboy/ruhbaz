...
- [ ] **B-1 Lunar cycle doğrulaması:** Astro motoru ay fazını "lokasyona göre" yanlış gösteriyordu; düzeltildi mi belirsiz. ÖNEMLİ TEŞHİS İPUCU: Ay fazı aslında lokasyondan BAĞIMSIZDIR (jeosantrik — aynı anda dünyanın her yerinde aynı faz); lokasyona göre değişen şey fazın YEREL TARİHE düşüşüdür (saat dilimi/gün sınırı) ve ay doğuş/batışıdır. Yani hata büyük olasılıkla kütüphanede değil, yerel tarih ↔ UTC dönüşümünde (örn. yeniay TR saatiyle gece 01:00'de ama hesap UTC günüyle yapılınca "dün" görünmesi). Kontrol noktası: astroEngine'de faz hesabına giden Date'in UTC mi yerel mi kurulduğu. Kütüphane: `astronomy-engine` (cosinekitty/astronomy, MIT lisans, Swiss Ephemeris KULLANMAZ — bilinçli seçimdi: Swiss Ephemeris GPL/ücretli ticari lisans gerektirir, MIT olan bu motor ticari app için doğru tercih; bu gerekçe unutulmasın). → ℹ️ **(2026-06-15 İNCELENDİ — düzeltilecek bug YOK):** Kodda ay-FAZI/illuminasyon özelliği YOK; Ay yalnız jeosantrik konum/burç olarak hesaplanıyor ve sky context tarihleri `...T12:00:00.000Z` (UTC öğlen) + `Astronomy.AstroTime` ile kuruluyor — jeosantrik için doğru, lokasyondan bağımsız. İleride ay-fazı/yeniay-dolunay özelliği EKLENİRSE, yerel-tarihe düşüş için UTC↔yerel kontrolü O ZAMAN yapılır; şu an aktif bir hata yok.
- [ ] **B-2 Efemeris zenginleştirme (tutulma + ikincil cisimler):** DOĞRU YAKLAŞIM: bu olguları LLM bilgisine/websearch'e bırakmak değil (maliyet + halüsinasyon), cihazdaki `astronomy-engine` ile DETERMİNİSTİK hesaplayıp prompta "bugünün gökyüzü olguları" bloğu olarak vermek. Flash-lite olguyu ÜRETMEZ, anlatır.
  - **MEVCUT DURUM (kod doğrulandı 2026-06-11 — B-2 kısmen yapılmış):** astroEngine ZATEN hesaplıyor → 10 ana cisim (Güneş, Ay, Merkür→Plüton) + **Kuzey/Güney Ay Düğümü** + **Lilith** (mean/yaklaşık) + **retrograd durumu** (boylam hızı < 0; prompta da yazılıyor, astroEngine.ts:682-690). Yani "retro/Lilith anılmıyor" ESKİMİŞ; bunlar var.
  - **GERÇEK EKSİK:** (a) **Tutulmalar** (güneş/ay) ayrı olay olarak yüzeye çıkmıyor — astronomy-engine tutulma aramasını destekler, günlük prompta "bugünlerde tutulma var mı" bloğu olarak BAĞLANMALI. (b) Düğüm/Lilith her okumada TUTARLI yüzeye çıkıyor mu, yalnız bazılarında mı — denetle (Ozan'ın "sadece Lilith durması" endişesi: hepsi eşit anılmalı, biri öne biri geri kalmamalı).
  - ✅ **KISMEN ÇÖZÜLDÜ (2026-06-15):** `astroEngine.buildSkyEventsContext(period)` eklendi → ay evresi + sıradaki yeni ay/dolunay (`MoonPhase`/`Illumination`/`SearchMoonQuarter`) + yaklaşan güneş/ay tutulmaları (`SearchGlobalSolarEclipse`/`SearchLunarEclipse`) + **aktif retro gezegen listesi** (eşit anılır) deterministik hesaplanıp kişisel + genel astro prompt'larına "## Gökyüzü Olayları" bloğu olarak enjekte ediliyor; dönem-horizonuna göre "BU DÖNEMDE" etiketi + hiyerarşi kuralı (ana cisimden sonra, kehanet dili yok). **KALAN (düşük öncelik):** Chiron/asteroidler (astronomy-engine'de yok, ayrı efemeris); birth-chart + astro-followup'a da yayma; ingress/mevsim olayları.
  - **EKLENEBİLİR ama astronomy-engine'de YOK (ayrı efemeris/yörünge verisi gerekir):** Chiron, asteroidler (Ceres/Pallas/Juno/Vesta). Maliyet/karmaşıklık vs. astrolojik katkı tartılır; ÖNCELİK DÜŞÜK (aşağıdaki ilke).
  - **⭐ ÖNCELİK İLKESİ (Ozan, 2026-06-11):** Ana gezegenler + Güneş + Ay BİRİNCİL. Düğümler, Lilith, retrolar, (eklenirse) Chiron/asteroidler İKİNCİL — yorumda ana cisimlerden SONRA gelir ve normalde arka planda kalır. İSTİSNA: bu ikincil cisim, o gün gerçekten ÖNEMLİ ve İLGİ ÇEKİCİ bir gökyüzü olayının (ör. düğüm ekseninde tutulma, sıkı bir dış gezegen retrosu/açısı) BAŞ AKTÖRÜ ise öne çıkarılır. Yani "her şeyi say" değil; hiyerarşi + olay-odaklı öne çıkarma.
...
- [ ] **B-6 Embedding maliyet ölçüm açığı (kod doğrulandı):** `/gemini-embed` proxy'si yanıtında token usage döndürmüyorsa, `memoryEmbeddingService.ts:73` kaydı atlıyor → embedding maliyeti ledger'da GÖRÜNMÜYOR (Ozan'ın "sayaçta 0" gözlemi). Düzelt: backend Gemini embed API'sinin döndürdüğü token sayısını geçirsin; output=0 doğru (embedding çıktı tokeni üretmez), input ölçülmeli. Yerel-vs-bulut embedding kararının (11) ÖN ŞARTI bu ölçüm. → ✅ **ÇÖZÜLDÜ (2026-06-15 doğrulandı):** Açık 2026-06-11'deki durumdu; o günden beri kapanmış. Bugün: `token_server.py` `/gemini-embed` (sat. 288-296) usage'ı DÖNDÜRÜYOR (usageMetadata yoksa `estimated_raw_tokens≥1` fallback) → `geminiEmbeddingService.embedGeminiText` usage'ı forward ediyor (sat. 75) → `memoryEmbeddingService.recordEmbeddingUsage` HER İKİ embed noktasında (sat. 96 + 178) çağrılıyor → ledger'a input token yazılıyor (output=0 doğru). `:73` erken-dönüş yalnız üç değer de 0 ise tetikler, backend ≥1 garanti ettiği için tetiklenmez. Kod değişikliği gerekmedi.
...
- [ ] **Doğum günü kutlama push'u (K54):** profilin doğum gününde kutlama bildirimi; ana profil daha şatafatlı, ek profiller daha sade. UI kutlama modu + yorum dokunuşuyla birlikte (07/K54).
